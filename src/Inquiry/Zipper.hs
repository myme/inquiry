...
insert x z@(Zipper (Just c) xs _) =
  z
    { zipperCurrent = Just x,
      zipperPrev = c : xs
    }

focus :: Lens' (Zipper a) (Maybe a)
focus = lens peek setM
  where
    setM z Nothing = z
    setM z (Just x) = set x z
